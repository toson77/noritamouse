...
void adachi_method_g(void)
{
	...
	while (1)
	{
		...
		if (nextdir == 1)
		{
			...
			if (exist_f_wall == 0)
			{
				turn_g(-90.0, TURN_OMEGA, 0, TURN_ALPHA, m_dir);
			}
			else
			{
				turn(-90.0, TURN_OMEGA, 0, TURN_ALPHA);
		...
		if (nextdir == 2)
		{
			...
			{
				g_current_angle = 270.0;
			}
			else if (m_dir == 2)
			{
				g_current_angle = 180.0;
			}
			else
			{
				g_current_angle = 90.0;
			}
		}
		//左折
		if (nextdir == 3)
...
void adachi_method_back_enable(void)
{
	...
	while (1)
	{
		...
		if (nextdir == 2)
		{
			...
			{
				turn(180.0, 300.0, 0, 600.0);
				revision_back(0.035, 0.3, 5);
			}
			straight(0.135, SEARCH_SPEED, SEARCH_SPEED, SEARCH_ACCEL, 0, 0);
